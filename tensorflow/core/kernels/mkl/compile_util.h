...
inline void compute_exp(__m512& x, __m512& rvar, __m512& mean_rvar, __m512& exp_t) {
  exp_t = _mm512_mul_ps(x, rvar);
  exp_t = _mm512_add_ps(exp_t, mean_rvar);
  exp_t = avx512_exp(exp_t);
}

inline __m512 compute_px(__m512& exp_t, __m512& alpha) {
  const __m512 p16f_1 = _mm512_set1_ps(1.0f);
  __m512 denominator, numerator;
  denominator = _mm512_add_ps(exp_t, alpha);
